...
#define       LTRPNCa     (*V).LTRPNCa
    ...
    ICaL = 0.31 * GCaL * sd * sf * sf2 * sfcass * 4 * (svolt - 15) * (F * F / (R * T)) *//Zhai change
        (0.25 * exp(2 * (svolt - 15) * F / (R * T)) * CaSS - Cao) / (exp(2 * (svolt - 15) * F / (R * T)) - 1.);
    Ito = Gto * sr * ss * (svolt - Ek);
    IKr = 0.3 * Gkr * sqrt(Ko / 5.4) * sxr1 * sxr2 * (svolt - Ek);//Zhai change
    IKs = 0.2 * Gks * sxs * sxs * (svolt - Eks);//Zhai change
