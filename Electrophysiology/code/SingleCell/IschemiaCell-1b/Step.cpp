...
#define       LTRPNCa     (*V).LTRPNCa
    ...
    ICaL = 0.5 * GCaL * sd * sf * sf2 * sfcass * 4 * (svolt - 15) * (F * F / (R * T)) *//Zhai change
        (0.25 * exp(2 * (svolt - 15) * F / (R * T)) * CaSS - Cao) / (exp(2 * (svolt - 15) * F / (R * T)) - 1.);
    Ito = Gto * sr * ss * (svolt - Ek);
    IKr = Gkr * sqrt(Ko / 5.4) * sxr1 * sxr2 * (svolt - Ek);
    IKs = Gks * sxs * sxs * (svolt - Eks);
    IK1 = GK1 * rec_iK1 * (svolt - Ek);
    INaCa = 0.6 * knaca * (1. / (KmNai * KmNai * KmNai + Nao * Nao * Nao)) * (1. / (KmCa + Cao)) *//Zhai change
        (1. / (1 + ksat * exp((n - 1) * svolt * F / (R * T)))) *
        (exp(n * svolt * F / (R * T)) * Nai * Nai * Nai * Cao -
            exp((n - 1) * svolt * F / (R * T)) * Nao * Nao * Nao * Cai * 2.5);
    INaK = 0.46 * knak * (Ko / (Ko + KmK)) * (Nai / (Nai + KmNa)) * rec_iNaK;//Zhai change
    IpCa = GpCa * Cai / (KpCa + Cai);
    IpK = GpK * rec_ipK * (svolt - Ek);
    IbNa = GbNa * (svolt - Ena);
    IbCa = 1.3 * GbCa * (svolt - Eca);//Zhai change
...
//根据：2011_Experiment-model interaction for analysis of epicardial activation during human ventricular fibrillation with global myocardial ischaemia
    ...
    double H = 2.0;                                                   
    double n = 0.24;                                                  
    double ATPi = 4.6;              //Zhai change_new    0.15;//
    double Khalf = 0.25;         //Zhai change_new       0.6676;//
    ...
    Irel = 0.65 * Vrel * sOO * (CaSR - CaSS);//Zhai change_new
    Ileak = Vleak * (CaSR - Cai);
    Iup = 0.9 * Vmaxup / (1. + ((Kup * Kup) / (Cai * Cai)));//Zhai change_new
