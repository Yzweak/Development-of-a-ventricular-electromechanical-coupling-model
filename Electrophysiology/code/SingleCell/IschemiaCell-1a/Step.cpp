...
#define       LTRPNCa     (*V).LTRPNCa
    ...
    //Compute currents
    INa = 0.887 * GNa * sm * sm * sm * sh * sj * (svolt - Ena);//Zhai change
    ICaL = 0.8 * GCaL * sd * sf * sf2 * sfcass * 4 * (svolt - 15) * (F * F / (R * T)) *//Zhai change
        (0.25 * exp(2 * (svolt - 15) * F / (R * T)) * CaSS - Cao) / (exp(2 * (svolt - 15) * F / (R * T)) - 1.);
    Ito = 0.5 * Gto * sr * ss * (svolt - Ek); //Zhai change
    IKr = Gkr * sqrt(Ko / 5.4) * sxr1 * sxr2 * (svolt - Ek);
    IKs = 0.781 * Gks * sxs * sxs * (svolt - Eks);  //Zhai change
    ...
    double ATPi = 4.6;              
    double Khalf = 0.25;        
    double IKATP = GKATP * (1 / (1 + pow(ATPi / Khalf, H))) * pow(Ko / 5.4, n) * (svolt - Ek);
    ...
    TAU_M = AM * BM;
    M_INF = 1. / ((1. + exp((-55.5 - svolt) / 9.03)) * (1. + exp((-55.5 - svolt) / 9.03)));
    if (svolt >= -40.)
    {
        AH_1 = 0.;
...
#ifdef EPI
    R_INF = 1. / (1. + exp((27.2 - svolt) / 6.));      //Zhai change
