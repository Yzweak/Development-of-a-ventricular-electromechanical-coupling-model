...
{
    ifstream in;
    in.open("/home/z/CardiacModels/MultiCells/parameters/matrix.txt",ios::in);
    if(!in){
...
{
    ifstream in;
    in.open("/home/z/CardiacModels/MultiCells/parameters/order.txt",ios::in);
    if(!in){
        printf("File read Failed!\n");
        return;
    }
...
void InitCanine(Atria_2D* , const int*);
...
void WriteBackFile(const double* t,Atria_2D* c0)
{
    char name1[300];
    char name2[300];
    sprintf(name1,"%s","ischemia-1b-calcium.dat");
    sprintf(name2,"%s","ischemia-1b-volt.dat");
...
void InitCanine(Atria_2D* c0, const int* order)
{
...
__device__ void get_Itot(Atria_2D* s0, int i, int j, double Istim)
{
    ...
    //Compute currents
    double INa = GNa * sm * sm * sm * sh * sj * (svolt - Ena);//liang_change
    double ICaL = 0.5 * GCaL * sd * sf * sf2 * sfcass * 4 * (svolt - 15) * (F * F / (R * T)) * (0.25 * exp(2 * (svolt - 15) * F / (R * T)) * CaSS - Cao) / (exp(2 * (svolt - 15) * F / (R * T)) - 1.);
    ...
    double INaCa = 0.6 * knaca * (1. / (KmNai * KmNai * KmNai + Nao * Nao * Nao)) * (1. / (KmCa + Cao)) * (1. / (1 + ksat * exp((n - 1) * svolt * F / (R * T)))) *
                   (exp(n * svolt * F / (R * T)) * Nai * Nai * Nai * Cao - exp((n - 1) * svolt * F / (R * T)) * Nao * Nao * Nao * Cai * 2.5);
    double INaK = 0.46 * knak * (Ko / (Ko + KmK)) * (Nai / (Nai + KmNa)) * rec_iNaK;
    double IpCa = GpCa * Cai / (KpCa + Cai);
    double IpK = GpK * rec_ipK * (svolt - Ek);
    double IbNa = GbNa * (svolt - Ena);
    double IbCa = 1.3 * GbCa * (svolt - Eca);
    ...
    double GNaL = 0.0075;
    double INaL = GNaL * (svolt - Ena) * mL * hL;
    ...
    double Irel = 0.65 * Vrel * sOO * (CaSR - CaSS);
    double Ileak = Vleak * (CaSR - Cai);
    double Iup = 0.9 * Vmaxup / (1. + ((Kup * Kup) / (Cai * Cai)));
    ...
    double dNai = -(INa + IbNa + 3 * INaK + 3 * INaCa) * inverseVcF * CAPACITANCE;
    s0->Nai[i][j] += HT * dNai;

    double dKi = -(Istim + IK1 + Ito + IKr + IKs - 2 * INaK + IpK) * inverseVcF * CAPACITANCE;
    s0->Ki[i][j] += HT * dKi;
