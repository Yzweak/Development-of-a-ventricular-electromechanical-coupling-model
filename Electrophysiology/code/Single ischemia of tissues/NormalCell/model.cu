...
void WriteBackFile(double* t,Atria_2D* c0)
{
    ...
    char name2[300];
    char name3[300];
    sprintf(name1,"%s","/home/z/CLionProjects/NormalCell/output/normal-calcium.dat");
    sprintf(name2,"%s","/home/z/CLionProjects/NormalCell/output/normal-volt.dat");
    sprintf(name3,"%s","/home/z/CLionProjects/NormalCell/output/normal-du.dat");
...
__device__ void get_Itot(Atria_2D* s0, int i, int j, double Istim)
{
    ...
    double ICaL = GCaL * sd * sf * sf2 * sfcass * 4 * (svolt - 15) * (F * F / (R * T)) * (0.25 * exp(2 * (svolt - 15) * F / (R * T)) * CaSS - Cao) / (exp(2 * (svolt - 15) * F / (R * T)) - 1.);
    ...
    if(cell_type == 1)
        s0->Itot[i][j] = IKr + IKs + IK1 + Ito + INa + IbNa + ICaL + IbCa + INaK + INaCa + IpCa + IpK + Istim;
    else
        s0->Itot[i][j] = IKr + IKs + IK1 + Ito + INa + IbNa + ICaL + IbCa + INaK + INaCa + IpCa + IpK;
