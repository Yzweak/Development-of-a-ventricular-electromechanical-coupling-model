...
void WriteBackFile(double* t,Atria_2D* c0)
{
    char name1[300];
    char name2[300];
    sprintf(name1,"%s","MI-short-calcium.dat");
    sprintf(name2,"%s","MI-short-volt.dat");
...
__device__ void get_Itot(Atria_2D* s0, int i, int j, double Istim)
{
    ...
    double ICaL = 0.62 * GCaL * sd * sf * sf2 * sfcass * 4 * (svolt - 15) * (F * F / (R * T)) * (0.25 * exp(2 * (svolt - 15) * F / (R * T)) * CaSS - Cao) / (exp(2 * (svolt - 15) * F / (R * T)) - 1.);
    double Ito = 0.37 * Gto * sr * ss * (svolt - Ek);
    double IKr = 0.30 * Gkr * sqrt(Ko / 5.4) * sxr1 * sxr2 * (svolt - Ek);
    double IKs = 0.20 * Gks * sxs * sxs * (svolt - Eks);
