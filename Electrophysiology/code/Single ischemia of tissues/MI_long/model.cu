...
void WriteBackFile(double* t,Atria_2D* c0)
{
    char name1[300];
    char name2[300];
    sprintf(name1,"%s","MI-long-calcium.dat");
    sprintf(name2,"%s","MI-long-volt.dat");
...
__device__ void get_Itot(Atria_2D* s0, int i, int j, double Istim)
{
    ...
    //Compute currents
    double INa = 0.38 * GNa * sm * sm * sm * sh * sj * (svolt - Ena);//liang_change
    double ICaL = 0.31 * GCaL * sd * sf * sf2 * sfcass * 4 * (svolt - 15) * (F * F / (R * T)) * (0.25 * exp(2 * (svolt - 15) * F / (R * T)) * CaSS - Cao) / (exp(2 * (svolt - 15) * F / (R * T)) - 1.);
    double Ito = Gto * sr * ss * (svolt - Ek);
    double IKr = 0.3 * Gkr * sqrt(Ko / 5.4) * sxr1 * sxr2 * (svolt - Ek);
    double IKs = 0.2 * Gks * sxs * sxs * (svolt - Eks);
