...
void readDirection(double** matrix, int mode)
{
    ifstream in;
    if(mode == 1)
        in.open(R"(C:\Users\dell\Desktop\CLionProjects\MixFiber\ventricle_file\dot_matrix.txt)",ios::in);
    if(!in){
        printf("File read Failed!\n");
        return;
    }
...
int main(int argc, char **argv)
{
    ...
    {
        if (time >= tbegin && time <= tend)   // && stims_count < 101
        {
            Istim = stimstrength;
            isStimed = true;
        }
    ...
    for (int j=0;j<YD;j++)
    {
        for (int i=0;i<XD;i++)
        {
            ...
            if(index < cellNum)
            {
                ...
                int c = 0;
                int d = 0;
                if (c0->state[i][j] == 2 || c0->state[i][j] == 3 || c0->state[i][j] == 4) factor = 0.7;
                //   only two neighbor
                if(index3 == -1)
                {
