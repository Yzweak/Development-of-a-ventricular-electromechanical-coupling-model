...
{
    ifstream in;
    in.open(R"(C:\Users\dell\Desktop\CLionProjects\MixVentricle\ventricle_file\matrix.txt)",ios::in);
    if(!in){
...
void readDirection(double** matrix, int mode)
{
    ifstream in;
    if(mode == 1)
        in.open(R"(C:\Users\dell\Desktop\CLionProjects\MixVentricle\ventricle_file\dot_matrix.txt)",ios::in);
    if(!in){
        printf("File read Failed!\n");
        return;
    }
...
void readFile(int* array, int mode)
{
    ifstream in;
    if(mode == 1)
        in.open(R"(C:\Users\dell\Desktop\CLionProjects\MixVentricle\ventricle_file\stim_cells.txt)",ios::in);
    else if(mode == 2)
        in.open(R"(C:\Users\dell\Desktop\CLionProjects\MixVentricle\ventricle_file\endo.txt)",ios::in);
    else if(mode == 3)
        in.open(R"(C:\Users\dell\Desktop\CLionProjects\MixVentricle\ventricle_file\myo.txt)",ios::in);
    else if(mode == 4)
        in.open(R"(C:\Users\dell\Desktop\CLionProjects\MixVentricle\ventricle_file\epi.txt)",ios::in);
    else if(mode == 5)
        in.open(R"(C:\Users\dell\Desktop\CLionProjects\MixVentricle\ventricle_state\ischemia-1a.txt)",ios::in);
    else if(mode == 6)
        in.open(R"(C:\Users\dell\Desktop\CLionProjects\MixVentricle\ventricle_state\ischemia-1b.txt)",ios::in);
    else if(mode == 7)
        in.open(R"(C:\Users\dell\Desktop\CLionProjects\MixVentricle\ventricle_state\MI-short.txt)",ios::in);
    else if(mode == 8)
        in.open(R"(C:\Users\dell\Desktop\CLionProjects\MixVentricle\ventricle_state\MI-long.txt)",ios::in);
    if(!in){
        printf("File read Failed!\n");
        return;
    }
...
void InitCanine(Atria_2D* , const int*, const int*, const int*, const int*,const int*, const int*, const int*, const int*);
...
void get_du(Atria_2D*,int**, double**);
...
int main(int argc, char **argv)
{
    ...
    auto** dot_matrix = new double*[rows];
    for(int i=0;i<rows;i++)
    {
        int* matrix_line = new int[cols];
        auto* dot_matrix_line = new double[4];
        matrix[i] = matrix_line;
        dot_matrix[i] = dot_matrix_line;
    }
    readMatrixFile(matrix);
    readDirection(dot_matrix,1);

    int* order = new int[stimNum];
    int* endo = new int[endoNum];
    int* myo = new int[myoNum];
    int* epi = new int[epiNum];
    readFile(order,1);
    readFile(endo,2);
    readFile(myo,3);
    readFile(epi,4);
    ...
    readFile(state_1a,5);
    readFile(state_1b,6);
    readFile(state_short,7);
    readFile(state_long,8);
    /*****   host   ************************************************/
    Atria_2D *cc;
    cc = new Atria_2D[1];
    InitCanine(cc,order,endo,myo,epi,state_1a,state_1b,state_short,state_long);
    ...
    {
        if (time >= tbegin && time <= tend && stim_count < 50)  // && stim_count < 34
        {
            Istim = stimstrength;
            isStim = true;
        }
...
//          Three progressively shortened stimulus intervals
//        if (time > tend)
//        {
//            if(isStim)
//                stim_count++;
...
//            isStim = false;
//        }
//          Five  progressively shortened stimulus intervals
//        if (time > tend)
//        {
//            if(isStim)
//                stim_count++;
//            Istim = 0.;
//            if(stim_count == 31) period = 500;
//            tbegin = tbegin + period;
//            tend = tbegin + stimduration;
//            isStim = false;
//        }
        // last period
        if(time >= 22000 && step % 50 == 0 )
        {
...
void WriteBackFile(const double* t,Atria_2D* c0)
{
    char name1[300];
    char name2[300];
    sprintf_s(name1,"%s",R"(C:\Users\dell\Desktop\CLionProjects\MixVentricle\output\calcium.dat)");
    sprintf_s(name2,"%s",R"(C:\Users\dell\Desktop\CLionProjects\MixVentricle\output\volt.dat)");
...
void InitCanine(Atria_2D* c0, const int* order, const int* endo, const int* myo, const int* epi, const int* state_1a, const int* state_1b, const int* state_short, const int* state_long)
{
    ...
    for (int y = 0; y < YD; y++) {
        for (int x = 0; x < XD; x++) {
            ...
            else
            {
                ...
                bool isEpi = false;
                //endo cell
                for (int i=0;i<endoNum;i++)
                {
                    if(temp == endo[i])
                    {
                        c0->location[x][y] = ENDO;
                        isEndo = true;
                        break;
                    }
                }
                if(isEndo) continue;
                //myo  cell
                for (int i=0;i<myoNum;i++)
                {
                    if(temp == myo[i])
                    {
                        c0->location[x][y] = MCELL;
                        isMyo = true;
                        break;
                    }
                }
...
void get_du(Atria_2D* c0,int** matrix, double** dot_matrix)
{
    for (int j=0;j<YD;j++)
    {
        for (int i=0;i<XD;i++)
        {
            ...
            if(index < cellNum)
            {
                ...
                int c = 0;
                int d = 0;
//                if (c0->state[i][j] == 1) factor = 0.58;
//                if (c0->state[i][j] == 2 || c0->state[i][j] == 3 || c0->state[i][j] == 4) factor = 0.408;
                if (c0->state[i][j] == 1) factor = 0.572;
                if (c0->state[i][j] == 2) factor = 0.412;
                if (c0->state[i][j] == 3) factor = 0.407;
                if (c0->state[i][j] == 4) factor = 0.404;

                //   only two neighbor
                if(index3 == -1)
                {
                    double Vm = c0->V[index%YD][index/YD];
                    double V1 = c0->V[index1%YD][index1/YD];
                    double V2 = c0->V[index2%YD][index2/YD];
                    for(int k=0;k<4;k++)
                    {
                        if (index == matrix[index1][k]) a = k;
                        if (index == matrix[index2][k]) b = k;
                    }
                    double du_1 = V1 >= Vm ? dot_matrix[index1][a] : dot_matrix[index][0];
                    double du_2 = V2 >= Vm ? dot_matrix[index2][b] : dot_matrix[index][1];
                    c0->du[i][j] = factor * Dlong * ((0.8*du_1+0.2)*(V1-Vm) + (0.8*du_2+0.2)*(V2-Vm)) / (dx*dx);
                }
                else
                {
                    if(index4 == -1)
                    {
                        ...
                        double V1 = c0->V[index1%YD][index1/YD];
                        double V2 = c0->V[index2%YD][index2/YD];
                        double V3 = c0->V[index3%YD][index3/YD];
                        for(int k=0;k<4;k++)
                        {
                            if (index == matrix[index1][k]) a = k;
                            if (index == matrix[index2][k]) b = k;
                            if (index == matrix[index3][k]) c = k;
                        }
                        double du_1 = V1 >= Vm ? dot_matrix[index1][a] : dot_matrix[index][0];
                        double du_2 = V2 >= Vm ? dot_matrix[index2][b] : dot_matrix[index][1];
                        double du_3 = V3 >= Vm ? dot_matrix[index3][c] : dot_matrix[index][2];
                        c0->du[i][j] = factor * Dlong * ((0.8*du_1+0.2)*(V1-Vm) + (0.8*du_2+0.2)*(V2-Vm) + (0.8*du_3+0.2)*(V3-Vm)) / (dx*dx);
                    }
                    else
                    {
                        double Vm = c0->V[index%YD][index/YD];
                        ...
                        double V2 = c0->V[index2%YD][index2/YD];
                        double V3 = c0->V[index3%YD][index3/YD];
                        double V4 = c0->V[index4%YD][index4/YD];
                        for(int k=0;k<4;k++)
                        {
                            if (index == matrix[index1][k]) a = k;
                            if (index == matrix[index2][k]) b = k;
                            if (index == matrix[index3][k]) c = k;
                            if (index == matrix[index4][k]) d = k;
                        }
                        double du_1 = V1 >= Vm ? dot_matrix[index1][a] : dot_matrix[index][0];
                        double du_2 = V2 >= Vm ? dot_matrix[index2][b] : dot_matrix[index][1];
                        double du_3 = V3 >= Vm ? dot_matrix[index3][c] : dot_matrix[index][2];
                        double du_4 = V4 >= Vm ? dot_matrix[index4][d] : dot_matrix[index][3];
                        c0->du[i][j] = factor * Dlong * ((0.8*du_1+0.2)*(V1-Vm) + (0.8*du_2+0.2)*(V2-Vm) + (0.8*du_3+0.2)*(V3-Vm) + (0.8*du_4+0.2)*(V4-Vm)) / (dx*dx);
                    }
                }
